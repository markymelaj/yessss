...
export default async function AdminPagosPage() {
  ...
  const { data } = await supabase.from('cuotas').select('*, perfiles(nombre_completo, identificador)').eq('estado', 'en_revision').order('updated_at', { ascending: false });
  ...
  return (
    <section className="card p-5">
      <p className="text-sm font-bold uppercase tracking-[0.22em] text-amber-300">Bandeja de revisión</p>
      <div className="mt-4 grid gap-4 lg:grid-cols-2">
        ...
        {cuotas.length === 0 ? <p className="muted text-sm">No hay comprobantes en revisión.</p> : null}
      </div>
    </section>
  );
}
